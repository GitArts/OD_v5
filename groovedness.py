...
def Analyse(MAX, MIN):
  """
  Rievu skaits - Ņemt vērā tikai kritumus;
  Zemākais iekritums - Ņemt vērā tikai kritumus
  """
  ...
  for i in range(np.shape(MAX)[0]):
    ...
    Y = abs(MAX[i, 0] - MIN[i, 0])   # <-- Y attālums starp pirmo MAX punktu un pirmo MIN punktu
    ...
    Y = abs(MAX[i, 0] - MIN[i+1, 0]) # <-- Y attālums starp pirmo MAX punktu un otro MIN punktu
...
def groove_main(pcd):
  ...
  # |=== Rievainības pakāpes novērtējums ===|
  print ("|=== Rievainīguma vērtības ===|\n", "Rievainīguma indekss =", Groove)
