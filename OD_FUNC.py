...
import numpy as np
from helper import get_pcd, show_pcd, Stat_removal, display_inlier_outlier, Find_corner, Vector
from Object_func import get_colorpoints, get_BoBox, From_BB_get_points

# |=== This function tries to get potential object ===|
def get_one_object(pcd, pcd_copy, projection_points, radius):
  ...
  # |=== err_track = True if sec_algorithm faild ===|
  ...
  for r in range(2, 300):
    ...
    if (np.size(New_points) == np.size(points[(points[:, 0] > obj_y+1)&
      ...
      if projection_points is None: projection_points = get_projection(pcd_copy)
      # |=== The second algorithm activation ===|
      OB_BB = _get_one_object(New_points, Y_max_row, projection_points)
      if OB_BB == None: return Vector(New_points), projection_points, True
      Size_could_be_0 = From_BB_get_points(OB_BB, points)
      New_points = Size_could_be_0 if np.size(Size_could_be_0)!=0 else New_points
      break
    else:
      continue
  pcd_out = Vector(New_points)
  #show_pcd(pcd_out)
  # return one possible object and projection points ================
  return pcd_out, projection_points, False # <-- pcd_out objekta analīzei un dzēšanai
					# <-- projection_points iet pa riņķi
...
def _get_one_object(points, Y_max_samples, projection_points):
  ...
  while True:
    ...
    if Done:
      ...
      #mesh = o3d.geometry.TriangleMesh.create_coordinate_frame(origin=Y_max_projection, size=10)
      projection_points = np.delete(projection_points, np.where(projection_points[:, 0] == Y_max_projection[0]), axis=0)
      #PRPR = Vector(projection_points)
      #show_pcd([PRPR, mesh])

  #mesh = o3d.geometry.TriangleMesh.create_coordinate_frame(origin=ob_dot, size=3)
  ...
  # Delay ====
  p_1_max_min = Find_corner(p_1, 111100)
